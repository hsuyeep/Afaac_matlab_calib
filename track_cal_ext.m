% Returning parameters from all iterations, for tracking calibration
% ghat (:, iter) = estimation of gain from last iteration,
% sigmahat (:, iter)= estimation of model source fluxes from last iter,
% Sigma_n = noise power estimation from last iteration
function [ghat, sigmahat, iter, Sigma_n] = track_cal_ext(Rhat, A, sigmas, mask, varargin)
...
sigmahat(:, 1) = sigmas;
Sigma_n = zeros(Nelem, Nelem, maxiter+1);
...
for iter = 2:maxiter+1
    ...
    sigmahat(:, iter) = real(inv(abs(conj(GA' * invR * GA)).^2) * diag(GA' * invR * (Rhat - Sigma_n(:,:, iter-1)) * invR * GA));
