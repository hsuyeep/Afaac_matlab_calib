...
function [srcflux, re_gain, im_gain] = pltcalsol (fname, nrecs, showplt)
	...
	% Log file for estimated fluxes
	flogname = strcat (fname, '.sigmas');
	if (exist (flogname) ~= 0) % File exists
		fprintf (2, 'File %s exists. Continue with overwrite [Enter] or Ctrl-C to quit.', flogname);
	end;
	fsigmas = fopen (flogname, 'w'); 
	fprintf (fsigmas, '%% Sigmas timeseries from solution file %s\n', fname);
	fprintf (fsigmas, '%% Generated by pltcalsol on %s\n', datestr (now));
	fprintf (fsigmas, '%% Time (MJDsec) CasAflux thoff(deg) phioff(deg)  CygAflux thoff(deg) phioff(deg) TauAflux  thoff(deg) phioff(deg) VirAflux  thoff(deg) phioff(deg) Sunflux thoff(deg) phioff(deg)\n');


	% Log file for calibrated gains
	flogname = strcat (fname, '.gains');
	if (exist (flogname) ~= 0) % File exists
		fprintf (2, 'File %s exists. Continue with overwrite [Enter] or Ctrl-C to quit.', flogname);
	end;
	fgain = fopen (flogname, 'w'); 
	fprintf (fgain, '%% Complex gain timeseries from solution file %s\n', fname);
	fprintf (fgain, '%% Generated by pltcalsol on %s\n', datestr (now));
	fprintf (fgain, '%% % Time (MJDsec) Ant (amp/ph) ...\n');
	...
	gainamp = zeros (1, Nelem);
	gainph  = zeros (1, Nelem);
	tstamp  = zeros (nrecs);
	...
	for ts = 1:nrecs;
		...
		% Populate log files: sigmas + src positions first
		fprintf (fsigmas, '%.2f ', rec.tobs);
		for src = 1:size (srcflux, 2)
			fprintf (fsigmas, '%f %f %f ', ...
					srcflux (ts, src), srcpos_th(ts,src), srcpos_phi(ts,src));
		end;
		fprintf (fsigmas, '\n');
		
		% Now for the gains
		fprintf (fgain, '%.2f ', rec.tobs);
		gainamp = hypot (re_gain(ts, :), im_gain (ts,:));
		gainph = atan2 (im_gain(ts,:), re_gain(ts,:));
		for src = 1:size (re_gain, 2)
			fprintf (fgain, '%f %f ', gainamp (src), gainph (src));
		end;
		fprintf (fgain, '\n');
...
%	title ('Histogram of stefcal iterations');
	...
	if (fgain == 1) fclose (fgain); end;
	if (fsigmas == 1) fclose (fsigmas); end;
