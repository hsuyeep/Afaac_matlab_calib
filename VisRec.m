...
classdef VisRec% < handle 
	...
	methods 
		...
		function info = getVisMeta (obj, info)
            ...
            fprintf (1, '<-- Record size : %d Bytes, datasize: %d floats', obj.recbytesize, obj.datfloatsize);
...
% 			obj.recdat.dt = meta.tend - meta.tstart;
			...
			dat = reshape (fread (obj.fid, obj.datfloatsize, 'single'), [41616, 63, 2, 2]);
			...
			if (pols(1)) 
				...
				obj.xx = complex (squeeze(dat(:,chans,1,1)), squeeze(dat(:,chans,1,2)));
			end;
			if (pols(4)) 
				% obj.recdat.yy = complex (squeeze(dat(:,chans,2,1)), squeeze(dat(:,chans,2,2)));
				obj.yy = complex (squeeze(dat(:,chans,2,1)), squeeze(dat(:,chans,2,2)));
			end;
